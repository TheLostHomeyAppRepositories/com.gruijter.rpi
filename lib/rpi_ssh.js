...
const getCPUTemp = 'for f in /sys/class/thermal/thermal_zone*; do '
  + 'type=$(cat "$f/type" 2>/dev/null); '
  + 'if [ "$type" = "cpu-thermal" ] || [ "$type" = "x86_pkg_temp" ]; then cat "$f/temp"; exit; fi; '
  + 'done; cat /sys/class/thermal/thermal_zone0/temp 2>/dev/null || echo 0';
...
const getProcUptime = 'cat /proc/uptime';
const getProcLoadAvg = 'cat /proc/loadavg';
const getProcStat = 'cat /proc/stat';

// used for user sessions
const getUsers = 'w';
const getLastLogin = 'last -10 --time-format iso'; // get last logged in/out users

// used for fan monitoring
const getCpuFanSpeed = 'cat /sys/devices/platform/cooling_fan/hwmon/*/fan1_input';
const getCpuFanPwm = 'cat /sys/devices/platform/cooling_fan/hwmon/*/pwm1';
...
class RPi {
  ...
  async getSysInfo() {
    ...
    if (!serial) {
      const fallbackUuid = await this.silentExec(
        'cat /sys/class/dmi/id/product_uuid 2>/dev/null || cat /sys/devices/virtual/dmi/id/product_uuid 2>/dev/null || hostname',
      ).catch(() => null);
      serial = fallbackUuid ? fallbackUuid.trim() : (hostName || 'unknown-device');
    }
  ...
  findLogsDelta(newLogs, oldLogsXXX) {
    ...
    const oldLoggedInNow = oldLogs.filter((logOld) => !logOld.logoutTime && !logOld.info.includes('gone')); // still logged in
    ...
    goneUsers = oldLoggedInNow
      .filter((oldLog) => {
        const match = newLogs.find((newLog) => oldLog.loginTime === newLog.loginTime);
        if (!match) {
          // The session is no longer in the last -10 history (pushed off),
          // so we cannot verify if they logged out. Do not trigger logout.
          return false;
        }
        // If it is in the history, check if it has logged out
        const isStillLoggedIn = !match.logoutTime && !match.info.includes('gone');
  ...
  async getLastLogin() {
    ...
    return this.parseLastLogin(lastLoginRaw);
  }
  ...
  async getStats() {
    ...
    // Fetch data in parallel batches to maintain speed without overwhelming the RPi.
    const parallelCommandMap = {
      ...
      memstat: getMemoryStats,
      uptimeRaw: getProcUptime,
      loadAvgRaw: getProcLoadAvg,
      statRaw: getProcStat,
      usersRaw: getUptime, // Fallback to extract connected users
      cpuFanSpeedRaw: getCpuFanSpeed,
      cpuFanPwmRaw: getCpuFanPwm,
    };

    const parallelCommandKeys = Object.keys(parallelCommandMap);
    const delimiter = '___RPISSH_DELIM___';

    // Combine all commands with delimiter echoes and end with true to guarantee exit code 0.
    const combinedCommand = `${parallelCommandKeys
      .map((key) => parallelCommandMap[key])
      .join(`\necho "${delimiter}"\n`)}\ntrue`;

    const stdout = await this.execute(combinedCommand);
    const parts = stdout.split(delimiter);
    const rawResults = {};
    parallelCommandKeys.forEach((key, index) => {
      rawResults[key] = parts[index] ? parts[index].trim() : null;
    });
    ...
    let uptime;
    if (uptimeRaw) {
      const upSecs = parseFloat(uptimeRaw.split(/\s+/)[0]);
      if (!Number.isNaN(upSecs)) {
        const days = Math.floor(upSecs / 86400);
        const hours = Math.floor((upSecs % 86400) / 3600);
        const minutes = Math.floor((upSecs % 3600) / 60);
        uptime = `${days > 0 ? `${days} days, ` : ''}${hours}:${minutes.toString().padStart(2, '0')}`;
      }
    }

    let users = 0;
    if (usersRaw) {
      const userMatch = usersRaw.match(/(\d+)\s+users?/);
    ...
    if (loadAvgRaw) {
      ...
      if (parts.length >= 4) {
        loadAvg = parseFloat(parts[0]);
        const procs = parts[3].split('/');
        if (procs.length === 2) {
          runningProcesses = parseInt(procs[0], 10);
          totalProcesses = parseInt(procs[1], 10);
        }
      }
    }

    let activeProcesses = runningProcesses || 0;
    let cpuUsage;
    if (statRaw) {
      const procsRunningMatch = statRaw.match(/procs_running\s+(\d+)/);
      const procsBlockedMatch = statRaw.match(/procs_blocked\s+(\d+)/);
      if (procsRunningMatch && procsBlockedMatch) {
        activeProcesses = parseInt(procsRunningMatch[1], 10) + parseInt(procsBlockedMatch[1], 10);
      }

      const cpuMatch = statRaw.match(/^cpu\s+(.*)/m);
      if (cpuMatch) {
        const times = cpuMatch[1].trim().split(/\s+/).map(Number);
        const idleTime = times[3] + (times[4] || 0); // idle + iowait
        const totalTime = times.reduce((a, b) => a + b, 0);

        if (this.lastCpuStat) {
          const deltaIdle = idleTime - this.lastCpuStat.idle;
          const deltaTotal = totalTime - this.lastCpuStat.total;
          if (deltaTotal > 0) cpuUsage = Math.round(100 * (1 - (deltaIdle / deltaTotal)));
        }
        this.lastCpuStat = { idle: idleTime, total: totalTime };
      }
    }

    let lastLogins = [];
    if (lastLoginRaw) lastLogins = this.parseLastLogin(lastLoginRaw);

    const cpuTempRawParsed = parseFloat(cpuTempRaw);
    const cpuTemp = !Number.isNaN(cpuTempRawParsed) ? cpuTempRawParsed / 1000 : null;

    let gpuTemp = null;
    if (gpuTempRaw) {
      const match = gpuTempRaw.match(/[-+]?\d*\.?\d+/);
      if (match) {
        const parsedGpu = Number(match[0]);
        if (!Number.isNaN(parsedGpu)) gpuTemp = parsedGpu;
      }
    }

    const { cpuMaxFreq } = this.sysInfo;
    let cpuScaling;
    if (cpuCurFreq && cpuMaxFreq) {
      const cur = parseFloat(cpuCurFreq);
      const max = parseFloat(cpuMaxFreq);
      if (!Number.isNaN(cur) && !Number.isNaN(max) && max > 0) {
        cpuScaling = Math.round(100 * (cur / max));
      }
    }
    ...
    if (memstat) {
      ...
      if (totalMatch) {
        ...
        if (availMatch) {
          ...
          if (totalMemory > 0) {
            const usage = Math.round(100 - (availableMemory / totalMemory) * 100);
            memUsage = Math.max(0, Math.min(100, usage));
          }
        } else {
    ...
    let storageUsage;
    if (rootStorageInfo) {
      // Find the percentage robustly, as formatting differs across distributions (BusyBox vs Coreutils)
      const lines = rootStorageInfo.trim().split('\n');
      const rootLine = lines.find((line) => line.trim().endsWith(' /'));
      if (rootLine) {
        const dataLine = rootLine.trim().split(/\s+/);
        const pctStr = dataLine.find((val) => val.includes('%'));
        if (pctStr) {
          const val = parseInt(pctStr.replace('%', ''), 10);
          if (!Number.isNaN(val)) {
            storageUsage = Math.max(0, Math.min(100, val));
          }
        }
      }
    }
    ...
    if (netDevInfo) {
      ...
      lines.forEach((line) => {
        ...
        const rx = parseInt(data[0], 10) || 0;
        const tx = parseInt(data[8], 10) || 0;
        const total = rx + tx;

        if (interfaceName.startsWith('wl') || interfaceName.startsWith('wlan')) {
          if (!bestWlan || total > bestWlan.total) {
            bestWlan = { rx, tx, total };
          }
        } else if (interfaceName.startsWith('eth') || interfaceName.startsWith('en') || interfaceName.startsWith('br-')) {
          if (!bestEth || total > bestEth.total) {
            bestEth = { rx, tx, total };
          }
        }
      });
      ...
      if (bestWlan) {
        WLAN0Traffic.rxBytes = bestWlan.rx;
        WLAN0Traffic.txBytes = bestWlan.tx;
      }
      if (bestEth) {
        ETH0Traffic.rxBytes = bestEth.rx;
        ETH0Traffic.txBytes = bestEth.tx;
      }
    }

    let cpuFanSpeed = null;
    if (cpuFanSpeedRaw) {
      const speed = parseInt(cpuFanSpeedRaw.trim(), 10);
      if (!Number.isNaN(speed)) cpuFanSpeed = speed;
    }

    let cpuFanPwm = null;
    if (cpuFanPwmRaw) {
      const pwm = parseInt(cpuFanPwmRaw.trim(), 10);
      if (!Number.isNaN(pwm)) cpuFanPwm = pwm;
    }
  ...
  async getGPIOStates() {
    const gpioStates = {};
    let method;
    let raw = await this.silentExec(getGPIOStatesNew).catch(() => null); // pinctrl
    if (raw) {
      method = 'pinctrl';
      ...
      raw = await this.silentExec(getGPIOStates); // raspi-gpio
      if (!raw) return { states: gpioStates, method: null };
    ...
    return { states: gpioStates, method };
  }

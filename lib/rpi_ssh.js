...
const getGPIOLevels = 'gpioget gpiochip0 0-53';
...
class RPi {
    ...
    if (netDevInfo) {
      ...
      lines.forEach((line) => {
        ...
        if (interfaceName.startsWith('wl') || interfaceName.startsWith('wlan')) {
          ...
          WLAN0Traffic.txBytes += parseInt(data[8], 10) || 0;
  ...
  async getGPIOLevels() {
    // This is an opportunistic, high-efficiency poll using the gpiod character device.
    // It relies on `gpiod` tools being installed and the common `gpiochip0` having up to 54 pins.
    // `silentExec` will return null if the command fails for any reason (not installed, permissions, etc.),
    // allowing the caller to fall back to a more robust method.
    const rawLevels = await this.silentExec(getGPIOLevels);
    if (!rawLevels) return null;

    try {
      const levels = rawLevels.trim().split(' ').map((level) => level === '1');
      const gpioLevels = {};
      levels.forEach((level, index) => {
        gpioLevels[index] = { level }; // Return in a format that can be merged
      });
      return gpioLevels;
    } catch (e) {
      return null; // In case of parsing errors, fallback to the heavy poll.
    }
  }

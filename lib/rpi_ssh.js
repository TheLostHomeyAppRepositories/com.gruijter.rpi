...
class RPi {
  ...
  async getStats() {
    ...
    if (wlanInfo) {
      const rxMatchWLAN = wlanInfo.match(/RX\s+(?:packets\s+\d+\s+)?bytes[:\s]+(\d+)/);
      if (rxMatchWLAN) WLAN0Traffic.rxBytes = parseInt(rxMatchWLAN[1], 10);
      const txMatchWLAN = wlanInfo.match(/TX\s+(?:packets\s+\d+\s+)?bytes[:\s]+(\d+)/);
      if (txMatchWLAN) WLAN0Traffic.txBytes = parseInt(txMatchWLAN[1], 10);
    }

    const ETH0Traffic = { rxBytes: 0, txBytes: 0 };
    if (ethInfo) {
      const rxMatchETH = ethInfo.match(/RX\s+(?:packets\s+\d+\s+)?bytes[:\s]+(\d+)/);
      if (rxMatchETH) ETH0Traffic.rxBytes = parseInt(rxMatchETH[1], 10);
      const txMatchETH = ethInfo.match(/TX\s+(?:packets\s+\d+\s+)?bytes[:\s]+(\d+)/);
      if (txMatchETH) ETH0Traffic.txBytes = parseInt(txMatchETH[1], 10);
    }
